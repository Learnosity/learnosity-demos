...
class SignatureUtils
{
    ...
    */
    ...
    private static function generatItemsPreHashString($objectToSign, $consumer_secret)
    {
        ...
        if(!isset($objectToSign['security']) || !count($objectToSign['security'])) {
            throw new InvalidArgumentException("Security block does not exist");
        }
        if(!isset($consumer_secret) || !strlen($consumer_secret)) {
            throw new InvalidArgumentException("consumer_secret was not passed in or was empty");
        }
        ...
        foreach ($signatureKeys as $key) {
            //Check key exists
            if(!isset($objectToSign['security'][$key]) || !strlen($objectToSign['security'][$key])) {
                throw new InvalidArgumentException('Signature block does not contain required key or key was empty:' . $key);
            }
