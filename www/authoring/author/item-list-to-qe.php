...
<script>
    ...
    function init () {
        authorApp.on('open:item', function (evt) {
            evt.preventDefault();
            var itemReference = evt.data.item.reference;
            var widgetReference = evt.data.item.metadata.authoring.regions[0].widget.references[0];
            authorApp.navigate('items/' + itemReference + '/widgets/' + widgetReference);
        });
    }
</script>
